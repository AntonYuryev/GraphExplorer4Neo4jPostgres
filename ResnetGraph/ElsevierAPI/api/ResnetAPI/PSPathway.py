...
class PSPathway(PSObject):
    ...
    def number_of_edges(self, rel_types:list[str]=[]):
      if rel_types:
          my_rels = [r for r in self.graph._psrels() if r.obget_type in rel_types]
          return len(my_rels)
    ...
    def to_xml_s(self,ent_props:list,rel_props:list,add_props2rel=dict(),add_props2pathway=dict(),format='RNEF',as_batch=True, prettify=True):
        ...
        if format == 'SBGN':
            from .rnef2sbgn import rnef2sbgn_str  # optional dep — only needed for SBGN export
    ...
    def filter_rels_by_year(self, min_year=None, max_year=None):
      filtered_graph = self.graph.filter_rels_by_year(min_year, max_year)
      return PSPathway(self,filtered_graph)


    def nodes(self):
      return self.graph._get_nodes()
