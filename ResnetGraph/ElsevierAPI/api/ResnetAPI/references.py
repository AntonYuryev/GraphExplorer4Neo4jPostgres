...
class Reference(dict):
  ...
  def add_snippet(self,textref:str,snippet:dict[str,set]):
    '''
    input:
      textref can be empty string, in which case a standard textref will be generated from self.Identifiers\n
      snippet = {prop_name:{values}}
    '''
    my_textref = str(textref).strip() if textref else self._make_textref()
    if my_textref in self.snippets:
      my_textref = self._make_textref(self.number_of_sentences()+1)
    [self.add_sentence_props(my_textref,k,list(v)) for k,v in snippet.items()]
    return my_textref
  ...
  def _make_standard_textref(self,count = 0):
    for identifier, prefix_format in IDENTIFIER_PREFIXES:
      value = self.Identifiers.get(identifier,'')
      if value:
        if count > 0:
          return prefix_format.format(value)+'#'+str(count)  
        else:
          return prefix_format.format(value)
    return ''


  def _make_non_standard_textref(self, count:int=0):
  ...
  def _make_textref(self, count:int=0):
      textref = self._make_standard_textref(count)
      return textref if textref else self._make_non_standard_textref(count)
